            ...
            local iter = function(a, i)
                while true do
					...
					if(descriptor ~= nil) then
						print("descriptor is "..descriptor.name)
						print("descriptor.cpp_type is"..descriptor.cpp_type)
						if(value ~= nil and type(value)~="table") then
							print("value:"..value) 
						end 
						if (value ~= nil and type(value) == "table") then
						  for k,v in pairs(value) do
						    print("k is " .. k .. "v is " .. tostring(v))
						  end
						end
					end 
                    --]]		
...
local function _AddSerializePartialToStringMethod(message_descriptor, message_meta)
    ...
    local _internal_serialize = function(self, write_bytes)
        ...
        for field_descriptor, field_value in message_meta._member.ListFields(self) do
		    print("entering _encoder")
			--if(field_descriptor ~= nil) then
			--	print("field_descriptor:"..field_descriptor.name)
			--	if(field_value ~= nil ) then
						--if(field_value.type=="table") then
						--	print("field_value:"..cjson.encoder(field_value))
						--else 
			...
			print("existing _encoder")
        end
		print("existing for loop")
    end
    ...
    local _serialize_partial_to_string = function(self)
		...
		print(" entering _internal_serialize")
        _internal_serialize(self, write)
		print(" exit _internal_serialize 3")
		--local returnvalue=concat(out)
		print(" call concat")
        return concat(out)
		--return "teerererer"
    end
