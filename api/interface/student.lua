...
-- 格式:
-- location ^~/interface/student/ {
--     default_type 'text/plain';
--     content_by_lua_file ${HOME_DIR}/interface/student.lua;
-- }
-- 浏览器请求的地址是 http://${DOMAIN}/interface/student/${OP}
-- DOMAIN 服务器部署的域名
-- OP的值取决于代码中的实现
...
-- RESTFUL API定义格式见 https://github.com/changyuezhou/api_base/blob/master/doc/student.md
-- *********************************************************************************************************
...
function check_add_params(tbl)
    ...
    if (nil == tbl["age"]) or
            ("number" ~= type(tbl["age"])) then
        return false, "请检查参数age.为必填且必须为int型"
    end
    if (nil == tbl["have_courses"]) or
            ("table" ~= type(tbl["have_courses"])) then
        return false, "请检查参数have_courses.为必填且必须为数组"
    end
    if (nil == tbl["course_grade"]) or
            ("table" ~= type(tbl["course_grade"])) then
        return false, "请检查参数course_grade.为必填且必须为数组"
    end
...
function check_update_params(tbl)
    ...
    if (nil == tbl["name"]) or
            ("string" ~= type(tbl["name"])) then
        return false, "请检查参数name.必须为字符型a"
    end

    if (nil == tbl["age"]) or
            ("number" ~= type(tbl["age"])) then
        return false, "请检查参数sge.必须为字符型"
    end
    if (nil == tbl["have_courses"]) or
            ("table" ~= type(tbl["have_courses"])) then
        return false, "请检查参数have_courses.必须为数组"
    end
    if (nil == tbl["course_grade"]) or
            ("table" ~= type(tbl["course_grade"])) then
        return false, "请检查参数have_courses.必须为数组"
...
if OP == "add" then
    ...
    if true == result then
        local business = require "student_add"
...
elseif OP == "update" then
    ...
    if true == result then
        local business = require "student_update"
...
elseif OP == "query" then
    ...
    if true == result then
        local business = require "student_query"
...
elseif OP == "delete" then
    ...
    if true == result then
        local business = require "student_delete"
...
elseif OP == "query_list" then
    ...
    if true == result then
        local business = require "student_query_list"
...
elseif OP == "stat_by_date" then
    ...
    if true == result then
        local business = require "student_stat_by_date"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
